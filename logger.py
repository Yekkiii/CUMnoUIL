...
class Logger(object):
    ...
    def __init__(self, runs, info=None):
        ...
        self.logger = logging.getLogger('log file')
        self.logger_setter(info.logger_file+str(date.today())+'_'+info.dataset+'_'+info.method+'_output'+info.save_index+'.log')
    
    def print(self,info):
        self.logger.info(info)

    def logger_setter(self,file_name):
        self.logger.setLevel(logging.INFO)
        log_hdls = logging.FileHandler(file_name,'w',encoding='utf-8')
        formatter = logging.Formatter('%(asctime)s - %(filename)s - line:%(lineno)d - %(levelname)s - %(message)s -%(process)s')
        log_hdls.setFormatter(formatter)
    ...
    def print_statistics(self, run=None, mode='max_acc'):
        if run is not None:
            ...
            if mode == 'max_acc':
                ...
                ind = argmin
            self.logger.info(f'Run {run + 1:02d}:')
            self.logger.info(f'Highest Train: {result[:, 0].max():.2f}')
            self.logger.info(f'Highest Valid: {result[:, 1].max():.2f}')
            self.logger.info(f'Highest Test: {result[:, 2].max():.2f}')
            self.logger.info(f'Chosen epoch: {ind+1}')
            self.logger.info(f'Final Train: {result[ind, 0]:.2f}')
            self.logger.info(f'Final Test: {result[ind, 2]:.2f}')
            ...
            self.logger.info(f'All runs:')
            r = best_result[:, 0]
            self.logger.info(f'Highest Train: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 1]
            self.logger.info(f'Highest Test: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 2]
            self.logger.info(f'Highest Valid: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 3]
            self.logger.info(f'  Final Train: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 4]
            self.logger.info(f'   Final Test: {r.mean():.2f} ± {r.std():.2f}')
